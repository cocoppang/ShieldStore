...
void file_to_memory() {
	...
	for(int i = 0; i < arg.num_threads; i++) {
		count[i] = 0;
		inst[i] = (char**)malloc(sizeof(char*)*arg.run_inst_size/arg.num_threads);
		for(int j = 0; j < arg.run_inst_size/arg.num_threads; j++)
			inst[i][j] = (char*)malloc(sizeof(char)*arg.max_buf_size);
	}

	//sprintf(filename, "/home/workloads/workload_%dB_%dB_new/tracea_run_%c.0.txt",
	//		arg.max_key_size, arg.max_val_size, arg.workload);	
	sprintf(filename, "traces/trace_run.txt");
	fprintf(stdout, "trace file name: %s\n",filename);
	fp = fopen(filename, "r");
	while(set_count < arg.run_inst_size) {
		if(set_count % (arg.run_inst_size/arg.num_threads) == 0 && set_count != 0) {
 ...
 */
	//arg.run_inst_size = 10000000;
	//arg.run_inst_size = 9999872;
	arg.run_inst_size = 500;
